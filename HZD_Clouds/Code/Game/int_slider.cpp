...
IntSlider::IntSlider(const char* title, int min, int max, int* bound)
    :SliderBase(title)
    ,m_min_value(min)
    ,m_max_value(max)
    ,m_bound_value(bound)
...
void IntSlider::update(float ds)
{
    UNUSED(ds);
...
void IntSlider::render(Vector2* out_cursor)
{
    ...
    if(m_is_dragging){
        ...
        *m_bound_value = ((int)MapFloatToRange((float)cur_value, 0.0f, 1.0f, (float)m_min_value, (float)m_max_value));
    }

    // Draw the circle where it goes in the range
    float radius = 0.0075f;
    float normalized_pos = MapFloatToRange((float)*m_bound_value, (float)m_min_value, (float)m_max_value, 0.0f, 1.0f);
    ...
    std::string cur_text = Stringf("%i", *m_bound_value);
...
void IntSlider::handle_mouse_down(float x, float y, float radius)
{
...
float IntSlider::get_width() 
{
    return 0.35f;
}

float IntSlider::get_height()
{
    return 0.070f;
}