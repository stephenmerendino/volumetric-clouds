...
float calculate_perlin_worley(float3 pos)
{
    ...
    perlin = range_map(perlin, 0.25f, 1.0f, 0.0, 1.0f);
...
float calculate_detail_worley_one(float3 pos)
{
    return 1.0f - get_worley_noise_3d(pos, DETAIL_WORLEY_ONE_SCALE, DETAIL_WORLEY_NUM_OCTAVES, DETAIL_WORLEY_OCTAVE_PERSISTENCE, DETAIL_WORLEY_OCTAVE_SCALE, true);
}

float calculate_detail_worley_two(float3 pos)
{
    return 1.0f - get_worley_noise_3d(pos, WORLEY_TWO_SCALE, DETAIL_WORLEY_NUM_OCTAVES, DETAIL_WORLEY_OCTAVE_PERSISTENCE, DETAIL_WORLEY_OCTAVE_SCALE, true);
}

float calculate_detail_worley_three(float3 pos)
{
    return 1.0f - get_worley_noise_3d(pos, WORLEY_THREE_SCALE, DETAIL_WORLEY_NUM_OCTAVES, DETAIL_WORLEY_OCTAVE_PERSISTENCE, DETAIL_WORLEY_OCTAVE_SCALE, true);
}
...
float4 FragmentFunction(geometry_to_fragment_t data) : SV_Target0
{
    ...
    float mapped_z = (float)data.layer / 127.0f;
    ...
    float worley_one    = calculate_detail_worley_one(norm_pos);
    float worley_two    = calculate_detail_worley_two(norm_pos);
    float worley_three  = calculate_detail_worley_three(norm_pos);

    return float4(worley_one, worley_two, worley_three, 1.0f);
}
